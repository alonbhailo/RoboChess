...
def detect(
    ...
    for i in range(n_layers):
        ...
        im = Image.fromarray(image["orig"])
        im.save(f"debug_detect_{i}.jpeg")
...
def compute_corners(image_object):
    ...
    debug.DebugImage(image_object.get_images()[0]["orig"]).points(
        square_corners, size=5, color=(0, 0, 255)
    ).points(board_corners, size=5, color=(0, 255, 0)).save("corner_points")
