...
def _piece_with_highest_prob(top_probs_by_type: list[tuple[list[float], int]]) -> int:
...
def infer_chess_pieces(
    ...
    while num_of_undetermined_squares > 0:
        ...
        if max_pieces_left[max_idx] > 0 and predicted_piece_list[square] is None:
        ...
        for square, piece_type in enumerate(predicted_piece_list):
            if piece_type is None:
                if _is_white_piece(probs_with_no_indices[square]):
                    predicted_piece_list[square] = _determine_most_probable_white_piece(
                        probs_with_no_indices, square
                    )
                else:
                    predicted_piece_list[square] = _determine_most_probable_black_piece(
                        probs_with_no_indices, square
                    )
