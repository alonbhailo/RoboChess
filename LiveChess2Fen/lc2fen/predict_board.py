...
def detect_input_board(board: np.ndarray) -> list[list[int]]:
    ...
    corners detected.
    """
    head = "LiveChess2Fen/data/dataset"
    tmp_dir = os.path.join(head, "tmp/")
    if os.path.exists(tmp_dir):
        shutil.rmtree(tmp_dir)
    os.mkdir(tmp_dir)
    image_object = detect(board)
    board_corners, _ = compute_corners(image_object)
    return board_corners, image_object


def obtain_individual_pieces(board: np.ndarray) -> list[str]:
    ...
    os.mkdir(pieces_dir)
    cropped_imgs, corners = split_board_image_trivial(board, "", pieces_dir)
    return cropped_imgs, corners
